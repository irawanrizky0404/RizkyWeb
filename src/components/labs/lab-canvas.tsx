...
const AudioVisualizer = dynamic(() => import("@/components/labs/audio-visualizer").then(m => m.AudioVisualizer), { ssr: false });
...
export function LabCanvas({ componentName }: { componentName: string }) {
  ...
  if (componentName === "AudioVisualizer") return <AudioVisualizer />;
  return null;
}

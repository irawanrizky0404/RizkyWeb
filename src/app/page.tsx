    ...
    <>
      ...
      <section className="border-t-2 border-signal px-5 py-14 md:px-8 md:py-20">
        <div className="flex items-start gap-5 md:gap-8">
          ...
          <div className="flex-1" style={{ textAlign: (design?.hero?.statementAlign || "left") as "left" | "center" | "right" }}>
            <MaskReveal delay={0.08}>
              <p className="dis text-white" style={{ fontSize: design?.hero?.statementSize || "clamp(2.4rem, 9vw, 13rem)", lineHeight: 0.85 }}>
                {design?.hero?.statement || homepage?.statement || "Working at the frequency between signal and silence."}
              </p>
            </MaskReveal>
          </div>
        </div>
        <div className="mt-10 grid grid-cols-1 gap-gap border-t border-rule pt-8 md:grid-cols-[auto_1fr_auto]">
          ...
          <RevealText delay={0.22}>
            <p className="text-sm leading-loose text-white/50 max-w-xl">
              {design?.hero?.bio || homepage?.bio || "5 years across 3D, motion, illustration, and identity. Every project begins with a feeling, not a brief. Aesthetic leads. Technique follows."}
            </p>
          </RevealText>
          <RevealText delay={0.28}>
            <span className="lab text-white/15 hidden md:block" style={{ fontSize: "0.52rem" }}>{design?.site?.location || homepage?.location || "Indonesia"} · Est. {design?.site?.established || homepage?.established || "2017"}</span>
          </RevealText>
        </div>
      </section>

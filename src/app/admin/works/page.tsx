...
const EMPTY: Project = {
  ...
  tags: [], cover: "", gallery: [], featured: false, type: "client", videoUrl: "",
};
      ...
      <div className="shrink-0 bg-black border-b border-rule px-5 py-4">
        <div className="flex items-center justify-between gap-4 mb-4">
          <div>
            ...
            <p className="dis text-white" style={{ fontSize: "clamp(1.5rem, 4vw, 2.5rem)", lineHeight: 0.88 }}>Works</p>
          </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
        <Link href={`/works/${work.slug}`} target="_blank" className="lab text-white/30 hover:text-white transition-colors" style={{ fontSize: "0.5rem" }}>↗</Link>
    ...
    <div className="flex-1 overflow-auto">
      ...
      <form onSubmit={submit} className="p-6 max-w-3xl">
        ...
        <div className="flex items-center gap-4 mt-8 pt-6 border-t border-rule">
          ...
          {!isNew && (
            <Link href={`/works/${initial.slug}`} target="_blank" className="lab text-white/20 hover:text-white ml-auto" style={{ fontSize: "0.55rem" }}>
              View live ↗
            </Link>
          )}
        </div>
